...
|---|---|---|---|
| `genre` | *Fuzzy-search fallback:* MusicBrainz-first (artist-level), Beatport backfill only if MusicBrainz missed — both tier 50, order-guarded not precedence-driven. Preceded by a Beatport-URL direct fetch and by client-supplied `legacy` values (see note above) | Same master-store value, read **store-first** — but SetBuilder is not a pure reader: on a store miss that leaves a genuine gap, and only with a connected DJ, pool import re-resolves genre through `enrich_track` (Beatport→Tidal — **not** MusicBrainz, which SetBuilder never calls) and writes it back at provider precedence (§3.2, §5(b)) | `enrichment_pipeline.py:521-568`; earlier writers at `:437-456`, `:653-659`; SetBuilder's re-resolve at `pool.py:262-289` (step 4), `pool.py:548-595` |
| `bpm` | *Fuzzy-search fallback:* Beatport-first, Tidal backup only if Beatport missed — both tier 50, order-guarded. Preceded by Tidal-URL and Spotify-URL direct fetches and by client-supplied `legacy` values (see note above) | Same master-store value, read **store-first**, with the same gap-fill re-resolve as `genre` (`_enrich_and_writeback`). Nothing in SetBuilder *derives* BPM locally — it only re-asks the same two providers | `enrichment_pipeline.py:531-604`; earlier writers at `:459-475`, `:496-519`, `:653-659`; SetBuilder's re-resolve at `pool.py:262-289` (step 4), `pool.py:548-595` |
| `musical_key` | Same cascade and same two earlier sources as `bpm` | Same as bpm, including the gap-fill re-resolve (stored Camelot-normalized via `camelot_code`, `pool.py:571-573`) | `enrichment_pipeline.py:531-604`; earlier writers at `:459-475`, `:496-519`, `:653-659`; SetBuilder's re-resolve at `pool.py:262-289` (step 4), `pool.py:548-595` |
