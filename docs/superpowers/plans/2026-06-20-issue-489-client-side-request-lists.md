...
**Behavior:**
- On mount / when `code`/`enabled`/`sortField`/`sortDirection`/`statusFilter` change: start a (debounced-immediate) `loadAllPages` via a fetcher built from `api.getRequests`. For `best_match`, pass `sort: 'best_match'` and DO NOT client-resort (render server order). For the 7 simple fields, fetch with the **default server order** (so chunks stitch deterministically) and client-sort the result; but since the set is fully loaded, the simplest correct approach is: fetch with NO `sort` (server default `date_requested DESC`) for simple fields, then `sortRequests` in memory. **Decision:** always fetch simple fields with `sort: 'date_requested', direction: 'desc'` for stable chunk stitching, then sort in memory; fetch `best_match` with `sort: 'best_match'` and skip in-memory sort.
